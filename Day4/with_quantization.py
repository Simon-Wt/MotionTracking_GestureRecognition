...
pandas.DataFrame(Acc, columns=['time', 'ax', 'ay', 'az'])

# Quantization:
S = 1.0/np.sqrt(2.0)
Points = [ [1, 0, 0], [ S, -S, 0], [0, -1, 0], [-S, S, 0],
		   [-1, 0, 0], [-S, -S, 0], [0, 1, 0], [S, S, 0],
		   [ 0, S, -S], [0, 0, -1], [0, -S, S],
		   [0, -S, -S], [0, 0, 1], [0, S, S],
		   [ S, 0, -S], [-S, 0, S],
		   [-S, 0, -S], [S, 0, S]
...
for i in range(len(Acc)):
	lacc = np.sqrt(sum(Acc[i][1:]*Acc[i][1:]))
	if lacc > 0.0:
		for j in range(1,4):
			Acc[i][j] = Acc[i][j] / lacc
		Q, dQ = None, 10.0
...
for i in range(1, len(Acc)):
    ...
    Spd[i, 0] = Acc[i][0] # Copy time stamp
    for j in range(1,4):
